...
public class ElevensBoard extends Board {
	...
	public boolean anotherPlayIsPossible() {
		List<Integer> Remain = cardIndexes();
		if (containsJQK(Remain) || containsPairSum11(Remain)) 
		{
		    return true;
		}
		return false;
	...
	private boolean containsPairSum11(List<Integer> selectedCards) {
	    for (int i = 0; i < selectedCards.size(); i++) 
	    {
	        ...
	        {
	            int index = selectedCards.get(j);
	            int index2 = selectedCards.get(i);
	            Card c = cardAt(index); 
	            Card d = cardAt(index2);
	            if (c.pointValue() + d.pointValue() == 11 && (i != j)) 
	            {
	                return true;
